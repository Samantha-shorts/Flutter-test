...
class _VideoPlayerState extends State<VideoPlayer> {
  ...
  Future<void> _setup() async {
    ...
    // NOTE: _setup呼び出される前にcontrollerのsetDataSourceが完了してしまい
    // setDataSourceで実行される自動で全画面に遷移する処理が行われても
    // ```
    // if (configuration.autoFullscreen) {
    //   enterFullscreen();
    // }
    // ```
    // _controllerListenerが発火しないことがある。
    // _setup → setDataSourceの順番で実行させたいが影響範囲が大きいためここで反映させる
    if (_isFullscreen != widget.controller.value.isFullscreen) {
